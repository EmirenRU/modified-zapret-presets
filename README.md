...
- Все заслуги отдайте упомянутым ранее людям и их репозиториям!!! Без них не было бы и этого и других репозиториев. 
- Всегда будьте аккуратными с тем, что вы скачиваете из других репозиториев.
- Антивирус может ругаться на WinDivert.dll, WinDivert64.sys, Winws.exe. Это нормально. Это можно обосновать на дорогие лицензии со стороны Microsoft и модификацией сетевого потока.

## Установка

1. Скачиваете архив с Release и распаковываете. **ВАЖНО СВЕРЯТЬ ХЭШИ СКАЧАННОГО ФАЙЛА С РЕЛИЗНОЙ ВЕРСИЕЙ ФАЙЛА.**

```cmd
Get-FileHash modified-zapret-presets.7z -Algorithm SHA256
```
...
- Вы можете обновить бинарники zapret, скачав из оригинального репозитория *bol-van* и поместив их в папку /bin.  

## Проблемы 

1. Отсутствие поддержки VPN, таких как Wireguard. (Пытаюсь понять отчего это идет)
2. Может случиться, что Youtube не будет загружаться (бесконечная прогрузка). В данном случае в services.bat -> Включаете RESET.bat и пытаетесь прогрузить страницу. (Вправду, я не знаю почему так случается.)

## Предложения для улучшения

1. Почистить от дупликатов и распределить нормально по list-xxx.txt
2. Найти адекватные стратегии для работы. В данном случае создаете свой батник с именем своего пользователя или чем-то ещё и вписываете туда стратегии.
3. Создать версию для Linux.
